...
namespace cv {
    namespace wechat_qrcode_ncnn {
        ...
        WeChatEngine::WeChatEngine(const String &detector_prototxt_path,
            ...
            if (!super_resolution_prototxt_path.empty() &&
                !super_resolution_caffe_model_path.empty()) {
                p->use_nn_sr_ = true;
        ...
        vector<string> WeChatEngine::detectAndDecode(InputArray img, OutputArrayOfArrays points) {
            CV_Assert(!img.empty());
        ...
        vector<string> WeChatEngine::decode(const Mat &img, vector<Mat> &candidate_points,
            ...
            for (auto &point : candidate_points) {
                ...
                auto scale_list = p->getScaleList(cropped_img.cols, cropped_img.rows);
                for (auto cur_scale : scale_list) {
                    Mat scaled_img =
                            p->super_resolution_model_->processImageScale(cropped_img, cur_scale,
                                                                          p->use_nn_sr_);
                    string result;
        ...
        vector<Mat> WeChatEngine::detect(const Mat &img) {
            auto points = vector<Mat>();
            if (p->use_nn_detector_) {
                // use cnn detector
                auto ret = p->applyDetector(img, points);
                CV_Assert(ret == 0);
            } else {
        ...
        int WeChatEngine::Impl::applyDetector(const Mat &img, vector<Mat> &points) {
            int img_w = img.cols;
            int img_h = img.rows;
        ...
        Mat WeChatEngine::Impl::cropObj(const Mat &img, const Mat &point, Align &aligner) {
...
// empirical rules
        vector<float> WeChatEngine::Impl::getScaleList(const int width, const int height) {
